...
func compactDiskMap(diskMap: [String]) -> [String] {
    ...
    while fileIndex >= 0 {
        let fileId = compactedMap[fileIndex]
        let fileLength = compactedMap.filter { $0 == fileId }.count
        //print(fileId, fileLength)
        
        freeSpaceIndex = 0
        var freeSpaceLength = 0
        while freeSpaceIndex < fileIndex {
            while freeSpaceIndex < fileIndex && compactedMap[freeSpaceIndex] != "." {
                freeSpaceIndex += 1
            }
            if freeSpaceIndex < fileIndex {
                freeSpaceLength = compactedMap[freeSpaceIndex..<compactedMap.count].prefix(while: { $0 == "." }).count
        ...
        if freeSpaceLength >= fileLength {
            var blocksMoved = 0
            for i in 0..<compactedMap.count {
                if compactedMap[i] == fileId {
                    compactedMap[freeSpaceIndex + blocksMoved] = fileId
                    compactedMap[i] = "."
                    blocksMoved += 1
                    if blocksMoved == fileLength {
                        break
                    }
                }
            }
